...
//       const backendUrl = `${API_BASE_URL}/api/help/request`;
...
export default function SendHelpRequestPanel() {
  ...
  async function sendHelpRequest() {
    ...
    try {
      ...
      console.warn(
        "⚠️ Could not get token. Sending request without token for testing.",
      );
    }
  ...
  return (
    <div className="w-full max-w-2xl bg-gradient-to-br from-slate-800 via-slate-800 to-slate-900 border border-red-900/30 p-5 rounded-2xl shadow-2xl text-white">
      ...
      <div className="flex items-center justify-center gap-2 mb-4 pb-3 border-b border-red-900/30">
        ...
        <h2 className="text-lg font-bold tracking-wide whitespace-nowrap">
          ...
          <div className="p-2 bg-yellow-500/20 border border-yellow-500/50 rounded-lg flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-yellow-300 mt-0.5">
                No incidents in {currentLocation}. Contact emergency services.
              </p>
            </div>
          </div>
        )}
        ...
        {filteredIncidents.length > 0 && (
          <div>
            ...
            <select
              value={selectedDisaster?.disasterId || ""}
              onChange={(e) => {
                const disaster = filteredIncidents.find(
                  (d) => d.disasterId === parseInt(e.target.value),
                );
                setSelectedDisaster(disaster);
              }}
            ...
            >
              ...
              {filteredIncidents.map((incident) => (
                <option
                  key={incident.disasterId}
                  value={incident.disasterId}
                  className="bg-slate-800"
                >
                  ID: {incident.disasterId} | {incident.type} •{" "}
                  {incident.location}
                </option>
              ))}
            </select>
